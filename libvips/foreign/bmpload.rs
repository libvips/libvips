...
use vips_ffi::{GType, VipsForeignLoad, VipsForeignLoadClass, INTERPRETATION_SRGB};
...
fn parse(b: &[u8]) -> Result<Bmp, &'static str> {
    ...
    // checked: stride * height can exceed u32 on 32-bit targets
    let data_end = stride.checked_mul(height).and_then(|s| s.checked_add(data_offset));
    if data_offset < 54 || data_end.is_none_or(|end| end > b.len()) {
...
fn read_file(load: *mut VipsForeignLoad) -> Result<Vec<u8>, String> {
...
unsafe extern "C" fn bmp_header(load: *mut VipsForeignLoad) -> c_int {
...
unsafe extern "C" fn bmp_load(load: *mut VipsForeignLoad) -> c_int {
...
unsafe extern "C" fn class_init(class: *mut VipsForeignLoadClass) {
    struct Suffs([*const c_char; 3]);
    unsafe impl Sync for Suffs {}
    static SUFFS: Suffs = Suffs([c".bmp".as_ptr(), c".dib".as_ptr(), ptr::null()]);

    // the usual C-style casts up the class hierarchy, sound for repr(C)
    let object_class = class as *mut vips::VipsObjectClass;
    let foreign_class = class as *mut vips::VipsForeignClass;
