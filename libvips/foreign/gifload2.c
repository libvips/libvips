...
typedef struct _VipsForeignLoadGif2 {
 ...
 */
static const char *
vips_foreign_load_gif2_errstr( int error_code )
{
#ifdef HAVE_GIFLIB_5
...
static void
vips_foreign_load_gif2_error_vips( VipsForeignLoadGif2 *gif, int error )
{
	VipsObjectClass *class = VIPS_OBJECT_GET_CLASS( gif );

	const char *message;

	if( (message = vips_foreign_load_gif2_errstr( error )) )
		vips_error( class->nickname, "%s", message ); 
}

static void
vips_foreign_load_gif2_error( VipsForeignLoadGif2 *gif )
{
	int error;
	...
	if( error ) 
		vips_foreign_load_gif2_error_vips( gif, error ); 
}

static void
vips_foreign_load_gif2_close( VipsForeignLoadGif2 *gif )
{
#ifdef HAVE_GIFLIB_5
	if( gif->file ) {
		int error; 
	...
	if( gif->file ) {
		if( DGifCloseFile( gif->file ) == GIF_ERROR ) 
			vips_foreign_load_gif2_error_vips( gif, GifLastError() ); 
		gif->file = NULL;
	}
#endif
...
static int
vips_foreign_load_gif2_open( VipsForeignLoadGif2 *gif, const char *filename )
{
	g_assert( !gif->file ); 
	g_assert( !gif->fp ); 
...
static int
vips_foreign_load_gif2_open_buffer( VipsForeignLoadGif2 *gif, InputFunc read_fn )
{
	g_assert( !gif->file ); 
...
static void
vips_foreign_load_gif2_dispose( GObject *gobject )
{
	VipsForeignLoadGif2 *gif = (VipsForeignLoadGif2 *) gobject;

	vips_foreign_load_gif2_close( gif ); 

	VIPS_FREE( gif->comment ); 

	G_OBJECT_CLASS( vips_foreign_load_gif2_parent_class )->
		dispose( gobject );
}

static VipsForeignFlags
vips_foreign_load_gif2_get_flags_filename( const char *filename )
{
...
static VipsForeignFlags
vips_foreign_load_gif2_get_flags( VipsForeignLoad *load )
{
	return( VIPS_FOREIGN_PARTIAL );
}

static gboolean
vips_foreign_load_gif2_is_a_buffer( const void *buf, size_t len )
{
...
static gboolean
vips_foreign_load_gif2_is_a( const char *filename )
{
	unsigned char buf[4];

	if( vips__get_bytes( filename, buf, 4 ) == 4 &&
		vips_foreign_load_gif2_is_a_buffer( buf, 4 ) )
		return( 1 );
...
static void
vips_foreign_load_gif2_render_line( VipsForeignLoadGif2 *gif,
 ...
 */
static int
vips_foreign_load_gif2_render( VipsForeignLoadGif2 *gif, 
		...
		for( i = 0; i < 4; i++ ) {
			...
			for( y = InterlacedOffset[i]; 
				...
				if( DGifGetLine( gif->file, gif->line, 
					...
					vips_foreign_load_gif2_error( gif ); 
					return( -1 ); 
				}

				vips_foreign_load_gif2_render_line( gif, 
...
static int
vips_foreign_load_gif2_extension_next( VipsForeignLoadGif2 *gif,
...
static int
vips_foreign_load_gif2_graphics_ext( VipsForeignLoadGif2 *gif, 
	GifByteType *extension )
{
	...
	while( extension != NULL ) 
		if( vips_foreign_load_gif2_extension_next( gif, &extension ) )
			return( -1 ); 
...
static int
vips_foreign_load_gif2_application_ext( VipsForeignLoadGif2 *gif,
	GifByteType *extension )
{
...
static int
vips_foreign_load_gif2_comment_ext( VipsForeignLoadGif2 *gif,
	GifByteType *extension )
{
...
static int
vips_foreign_load_gif2_extension( VipsForeignLoadGif2 *gif )
{
	GifByteType *extension;
	int ext_code;
	...
	if( extension ) 
		...
		case GRAPHICS_EXT_FUNC_CODE: 
			if( vips_foreign_load_gif2_graphics_ext( gif,
				extension ) )
				return( -1 ); 
			break;

		case APPLICATION_EXT_FUNC_CODE: 
			if( vips_foreign_load_gif2_application_ext( gif, 
				extension ) )
				return( -1 ); 
			break;

		case COMMENT_EXT_FUNC_CODE:
			if( vips_foreign_load_gif2_comment_ext( gif, extension ) )
				return( -1 ); 
			break;
		...
		default:
			 ...
			 */
			while( extension != NULL ) 
				if( vips_foreign_load_gif2_extension_next( gif, 
					&extension ) )
					return( -1 ); 
			break;
		}
 ...
 */
static int
vips_foreign_load_gif2_page( VipsForeignLoadGif2 *gif, 
	VipsImage *previous, VipsImage *out )
{
	...
	do { 
		...
		case IMAGE_DESC_RECORD_TYPE:
			...
			if( vips_foreign_load_gif2_render( gif, previous, out ) )
				return( -1 ); 
		...
		case EXTENSION_RECORD_TYPE:
			if( vips_foreign_load_gif2_extension( gif ) )
				return( -1 );
			break;
...
static VipsImage *
vips_foreign_load_gif2_new_page( VipsForeignLoadGif2 *gif )
{
	VipsImage *out;
 ...
 */
static int
vips_foreign_load_gif2_pages( VipsForeignLoadGif2 *gif, VipsImage **out )
{
	VipsObjectClass *class = VIPS_OBJECT_GET_CLASS( gif );
	 ...
	 */
	if( !(frame = vips_foreign_load_gif2_new_page( gif )) ) 
		return( -1 );
	do { 
		if( vips_foreign_load_gif2_page( gif, NULL, frame ) ) {
	...
	while( gif->n == -1 ||
		...
		if( vips_foreign_load_gif2_page( gif, previous, frame ) ) {
			g_object_unref( frame ); 
			unref_array( frames );
			return( -1 );
		}
...
static int
vips_foreign_load_gif2_load( VipsForeignLoad *load )
{
	VipsForeignLoadGif2 *gif = (VipsForeignLoadGif2 *) load;
	VipsImage **t = (VipsImage **) 
		vips_object_local_array( VIPS_OBJECT( load ), 4 );

	VipsImage *im;

	if( vips_foreign_load_gif2_pages( gif, &t[0] ) )
		return( -1 );
	im = t[0];
...
static void
vips_foreign_load_gif2_class_init( VipsForeignLoadGif2Class *class )
{
	GObjectClass *gobject_class = G_OBJECT_CLASS( class );
	VipsObjectClass *object_class = (VipsObjectClass *) class;
	VipsForeignLoadClass *load_class = (VipsForeignLoadClass *) class;

	gobject_class->dispose = vips_foreign_load_gif2_dispose;
	...
	load_class->get_flags_filename = 
		vips_foreign_load_gif2_get_flags_filename;
	load_class->get_flags = vips_foreign_load_gif2_get_flags;

	VIPS_ARG_INT( class, "page", 20,
		_( "Page" ),
		_( "Load this page from the file" ),
		VIPS_ARGUMENT_OPTIONAL_INPUT,
		G_STRUCT_OFFSET( VipsForeignLoadGif2, page ),
		0, 100000, 0 );
	...
	VIPS_ARG_INT( class, "n", 21,
		...
		VIPS_ARGUMENT_OPTIONAL_INPUT,
		G_STRUCT_OFFSET( VipsForeignLoadGif2, n ),
		-1, 100000, 1 );

}

static void
vips_foreign_load_gif2_init( VipsForeignLoadGif2 *gif )
{
	gif->n = 1;
	gif->transparency = -1;
	gif->delay = 4;
...
typedef struct _VipsForeignLoadGif2File {
	VipsForeignLoadGif2 parent_object;

	/* Filename for load.
	 */
	char *filename; 

} VipsForeignLoadGif2File;

typedef VipsForeignLoadGif2Class VipsForeignLoadGif2FileClass;

G_DEFINE_TYPE( VipsForeignLoadGif2File, vips_foreign_load_gif2_file, 
	vips_foreign_load_gif2_get_type() );

static int
vips_foreign_load_gif2_file_header( VipsForeignLoad *load )
{
	VipsForeignLoadGif2 *gif = (VipsForeignLoadGif2 *) load;
	VipsForeignLoadGif2File *file = (VipsForeignLoadGif2File *) load;

	if( vips_foreign_load_gif2_open( gif, file->filename ) ) 
		return( -1 ); 

	VIPS_SETSTR( load->out->filename, file->filename );

	return( vips_foreign_load_gif2_load( load ) );
}
...
static void
vips_foreign_load_gif2_file_class_init( 
	VipsForeignLoadGif2FileClass *class )
{
	GObjectClass *gobject_class = G_OBJECT_CLASS( class );
	VipsObjectClass *object_class = (VipsObjectClass *) class;
	...
	object_class->nickname = "gifload2";
	object_class->description = _( "load GIF with giflib" );

	foreign_class->suffs = vips_foreign_gif_suffs;

	load_class->is_a = vips_foreign_load_gif2_is_a;
	load_class->header = vips_foreign_load_gif2_file_header;

	VIPS_ARG_STRING( class, "filename", 1, 
		_( "Filename" ),
		_( "Filename to load from" ),
		VIPS_ARGUMENT_REQUIRED_INPUT, 
		G_STRUCT_OFFSET( VipsForeignLoadGif2File, filename ),
		NULL );

}

static void
vips_foreign_load_gif2_file_init( VipsForeignLoadGif2File *file )
{
}

typedef struct _VipsForeignLoadGif2Buffer {
	VipsForeignLoadGif2 parent_object;
...
} VipsForeignLoadGif2Buffer;

typedef VipsForeignLoadGif2Class VipsForeignLoadGif2BufferClass;

G_DEFINE_TYPE( VipsForeignLoadGif2Buffer, vips_foreign_load_gif2_buffer, 
	vips_foreign_load_gif2_get_type() );
...
{
	VipsForeignLoadGif2Buffer *buffer = 
		(VipsForeignLoadGif2Buffer *) file->UserData;
...
static int
vips_foreign_load_gif2_buffer_header( VipsForeignLoad *load )
{
	VipsForeignLoadGif2 *gif = (VipsForeignLoadGif2 *) load;
	VipsForeignLoadGif2Buffer *buffer = (VipsForeignLoadGif2Buffer *) load;
	...
	if( vips_foreign_load_gif2_open_buffer( gif, vips_giflib_buffer_read ) ) 
		return( -1 ); 
...
static void
vips_foreign_load_gif2_buffer_class_init( 
	VipsForeignLoadGif2BufferClass *class )
{
	GObjectClass *gobject_class = G_OBJECT_CLASS( class );
	VipsObjectClass *object_class = (VipsObjectClass *) class;
	VipsForeignLoadClass *load_class = (VipsForeignLoadClass *) class;
	...
	object_class->nickname = "gifload2_buffer";
	object_class->description = _( "load GIF with giflib" );

	load_class->is_a_buffer = vips_foreign_load_gif2_is_a_buffer;
	load_class->header = vips_foreign_load_gif2_buffer_header;

	VIPS_ARG_BOXED( class, "buffer", 1, 
		_( "Buffer" ),
		_( "Buffer to load from" ),
		VIPS_ARGUMENT_REQUIRED_INPUT, 
		G_STRUCT_OFFSET( VipsForeignLoadGif2Buffer, buf ),
		VIPS_TYPE_BLOB );

}

static void
vips_foreign_load_gif2_buffer_init( VipsForeignLoadGif2Buffer *buffer )
{
}
 ...
 */
int
vips_gifload2( const char *filename, VipsImage **out, ... )
{
	va_list ap;
	int result;

	va_start( ap, out );
	result = vips_call_split( "gifload2", ap, filename, out );
	va_end( ap );
 ...
 */
int
vips_gifload2_buffer( void *buf, size_t len, VipsImage **out, ... )
{
	va_list ap;
	VipsBlob *blob;
	int result;
	...
	va_start( ap, out );
	result = vips_call_split( "gifload2_buffer", ap, blob, out );
	va_end( ap );
