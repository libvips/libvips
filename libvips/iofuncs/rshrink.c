 ...
 * only. No alpha handling. 11 x 11 mask. 
 *
...
/* Lanczos3 values for a x2 reduce generated by reduceh.cpp. This includes a
 * 0.5 pixel displacement up and left for centre sampling. 
 *
 *     0.00368914 0.0150561 -0.0339986 -0.0666373 0.135505 0.446385 0.446385
 *     0.135505 -0.0666373 -0.0339986 0.0150561 0.00368914 0 
 *         15 62 -139 -273 555 1828 1828 555 -273 -139 62 15 0 
 *
 */
#define SHRINK_INT_LANCZOS3_SHIFT_VER( TYPE, MN, MX ) { \
	TYPE * restrict tp = (TYPE *) p; \
	TYPE * restrict tq = (TYPE *) q; \
	\
	for( x = 0; x < in_cols; x++ ) { \
...
#define SHRINK_FLOAT_LANCZOS3_SHIFT_VER( TYPE ) { \
	TYPE * restrict tp = (TYPE *) p; \
	TYPE * restrict tq = (TYPE *) q; \
	\
	for( x = 0; x < in_cols; x++ ) { \
		tq[x] = 0.00368914 * tp[0] + \
		        0.0150561 * tp[1 * ls] + \
		       -0.0339986 * tp[2 * ls] + \
		       -0.0666373 * tp[3 * ls] + \
                        0.1355050 * tp[4 * ls] + \
...
#define SHRINK_INT_LANCZOS3_SHIFT_HOR( TYPE, MN, MX ) { \
	TYPE * restrict tp = (TYPE *) p; \
	TYPE * restrict tq = (TYPE *) q; \
	\
	for( x = 0; x < target->width; x++ ) { \
...
#define SHRINK_FLOAT_LANCZOS3_SHIFT_HOR( TYPE ) { \
	TYPE * restrict tp = (TYPE *) p; \
	TYPE * restrict tq = (TYPE *) q; \
	\
	for( x = 0; x < target->width; x++ ) { \
 ...
 */
static void
vips_region_shrink_uncoded_lanczos3_shift( VipsRegion *from, VipsRegion *to, 
	VipsRect *target, int margin )
{
	int ls = VIPS_REGION_LSKIP( from );
	int es = VIPS_IMAGE_SIZEOF_ELEMENT( from->im );
	...
	for( y = 0; y < target->height; y++ ) {
		...
		switch( from->im->BandFmt ) {
		case VIPS_FORMAT_UCHAR:	
			SHRINK_INT_LANCZOS3_SHIFT_VER( unsigned char, 
				0, UCHAR_MAX );  
			break; 

		case VIPS_FORMAT_CHAR:	
			SHRINK_INT_LANCZOS3_SHIFT_VER( signed char, 
				SCHAR_MIN, SCHAR_MAX );  
			break; 

		case VIPS_FORMAT_USHORT:	
			SHRINK_INT_LANCZOS3_SHIFT_VER( unsigned short, 
				0, USHRT_MAX );  
			break; 

		case VIPS_FORMAT_SHORT:	
			SHRINK_INT_LANCZOS3_SHIFT_VER( signed short, 
				SHRT_MIN, SHRT_MAX );  
			break; 

		case VIPS_FORMAT_UINT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_VER( unsigned int );  
			break; 

		case VIPS_FORMAT_INT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_VER( signed int );  
			break; 

		case VIPS_FORMAT_FLOAT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_VER( float );  
			break; 

		case VIPS_FORMAT_DOUBLE:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_VER( double );  
			break; 
	...
	for( y = 0; y < target->height; y++ ) {
		...
		switch( from->im->BandFmt ) {
		case VIPS_FORMAT_UCHAR:	
			SHRINK_INT_LANCZOS3_SHIFT_HOR( unsigned char, 
				0, UCHAR_MAX );  
			break; 

		case VIPS_FORMAT_CHAR:	
			SHRINK_INT_LANCZOS3_SHIFT_HOR( signed char, 
				SCHAR_MIN, SCHAR_MAX );  
			break; 

		case VIPS_FORMAT_USHORT:	
			SHRINK_INT_LANCZOS3_SHIFT_HOR( unsigned short, 
				0, USHRT_MAX );  
			break; 

		case VIPS_FORMAT_SHORT:	
			SHRINK_INT_LANCZOS3_SHIFT_HOR( signed short, 
				SHRT_MIN, SHRT_MAX );  
			break; 

		case VIPS_FORMAT_UINT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_HOR( unsigned int );  
			break; 

		case VIPS_FORMAT_INT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_HOR( signed int );  
			break; 

		case VIPS_FORMAT_FLOAT:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_HOR( float );  
			break; 

		case VIPS_FORMAT_DOUBLE:	
			SHRINK_FLOAT_LANCZOS3_SHIFT_HOR( double );  
			break; 
...
/**
 * vips_region_shrink_lanczos3_shift:
 * @from: source region 
 * @to: destination region 
 * @target: #VipsRect of @to to write
 * @margin: @target is offset by this much in @to
 ...
 * Write to the pixels in @target in @to using the in @from as the source.
 * @target includes a margin of @margin pixels. Non-complex uncoded images 
 * only. No alpha handling. This function uses a 12x12 mask. 
 *
 * We need a margin, since we want to be able to have some space in the output
 * at the left/top so we can call this function repeatedly.
 *
 ...
 */
int
vips_region_shrink_lanczos3_shift( VipsRegion *from, VipsRegion *to, 
	VipsRect *target, int margin )
{
	VipsImage *image = from->im;
	...
	/* Lanczos3 with a shift is 12x12.
	 */
	source.left = (target->left - margin) * 2;
	source.top = (target->top - margin) * 2;
	source.width = target->width * 2 + 11;
	source.height = target->height * 2 + 11;
	if( !vips_rect_includesrect( &from->valid, &source ) ) {
		printf( "argh not enough pixels!!\n" );
		return( -1 ); 
	}

	vips_region_shrink_uncoded_lanczos3_shift( from, to, target, margin );
